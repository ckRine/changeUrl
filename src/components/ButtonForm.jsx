...
const ButtonForm = ({ button, onSubmit, closeForm }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    conditionalLog(t("logs.formSubmit"), {
      name,
      replacements,
      params,
      actionType,
    ...
    if (!name.trim()) {
      ...
      conditionalLog(t("errors.nameEmpty"));
      return;
    }
    ...
    for (const { pattern, useRegex } of replacements) {
      if (pattern.trim() && useRegex) {
        try {
          ...
          conditionalLog(t("errors.regexSubmit"), error, pattern);
          return;
        }
      }
    }
    ...
    try {
      if (typeof onSubmit !== "function") {
        throw new Error(t("errors.onSubmitNotFunction"));
      }
      onSubmit(buttonData);
      toast.success(button ? t("success.buttonUpdated") : t("success.buttonAdded"));
      conditionalLog(t("logs.dataSubmitted"), buttonData);
    } catch (error) {
